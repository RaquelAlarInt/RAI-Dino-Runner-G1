...
from dino_runner.utils.constants import DEFAULT_TYPE, DUCKING,DUCKING_SHIELD, JUMPING, JUMPING_SHIELD, RUNNING,RUNNING_SHIELD
...
class Dinosaur(Sprite):
    ...
    def ducking(self):
        self.image = DUCKING_IMG[self.type] if self.step < 5 else DUCKING[1]
        self.step += 1
        self.rect.y = self.POS_Y_DUCK

    def run(self):
        self.image = RUNNING_IMG[self.type] if self.step < 5 else RUNNING[1]
        self.step += 1
    ...
    def draw_power_up(self, screen):
        if self.type != DEFAULT_TYPE:
            ...
            if time_to_show >= 0:
                draw_massages(f"{self.type.capitalize()} enabled for {time_to_show} seconds", 
                screen, 
                font_size = 22,
                pos_y_center = 50)
            else:
